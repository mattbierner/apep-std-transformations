...
describe('dicti', function () {
    it('Should extend pep.', () => {
        const pep2 = pep_trans(pep);
     
        const p = pep2.dicti({'a': 'b'});
        assert.deepStrictEqual(['b'], Array.from(p('a')));
    });

    it('Should map single value without case.', () => {     
        const mapper = pep_trans.dicti({
            'a': 'apple',
            'b': 'banana',
            'c': 'cookie'
        }, false);
    ...
    it('Map each value in a sequence without case.', () => {     
        const mapper = pep_trans.dicti({
            'a': 'apple',
            'b': 'banana',
            'aa': 'aardvark'
        ...
        assert.deepStrictEqual(
            ['apple', 'aardvark', 'banana', 'aardvark', false, false],
            Array.from(mapper(['A', 'AA', 'b', 'aA', 'c', 'ab'])));
    });
});
